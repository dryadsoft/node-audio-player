...
`/attendance`에서 **연도·학기 → 센터 → 요일 → 교시 → 페이지**을 선택합니다. 기존 강의계획서의 센터 목록을 공유합니다. 같은 학기에 같은 센터를 요일별로 등록할 수 있으며, 각 센터·요일의 교시 이름·개수·순서는 독립적으로 관리합니다. 같은 학기·센터·요일 조합은 휴지통을 포함해 하나만 등록할 수 있습니다. 시간 입력, OCR, 출석 통계, PDF 및 인쇄 기능은 없습니다.

관리 화면과 센터 사용 여부의 상세 동작은 [관리 모달 안내](management-ui.md)를 참고합니다.

## 사용

1. 선택 패널의 **출석부 관리**에서 관리 모달을 열고 센터·요일·교시를 준비합니다. 공통 센터 관리에서 사용 여부를 바꿀 수 있으며, 사용 중지 센터는 일반 선택 목록에서 숨기고 기존 기록은 보존합니다. **이 학기를 오프라인 저장 대상으로 지정**하면 해당 기기에서 그 학기 전체 센터·교시를 내려받습니다. 조회 학기와 자동 저장 대상은 별개입니다.
